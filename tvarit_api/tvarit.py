...
from .resources import (Admin, Dashboard, Datasource, Folder, Organization, Organizations, Search, User, Users, Teams)


class Tvarit:
    def __init__(self, auth, host="localhost", port=None, url_path_prefix="", protocol="http", verify=True):
        self.api = TvaritAPI(auth, host=host, port=port, url_path_prefix=url_path_prefix, protocol=protocol, verify=verify)
